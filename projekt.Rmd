...
##TODO
uzeli smo prvi model, testirali smo koje su vrijable znacajne
na temelju tih p vrijednosti smo se odlučili maknuti df,
testirali smo taj drugi bez df ( prikazati podatke),
testirali smo anovom jesu li modeli jednako dobri, jese pa mozemo maknuti df iz modela,
onda smo maknuli jos dvije varijable i opet testirali koje su u tom modelu znacajne varijable,
i usporedili taj model s ovim koji nema df, i dobili smo da su razliciti i ovaj drugi ima manje reziduale
pa mozemo uzeti ovaj drugi model kao bolji
onda smo ostavili samo najznacajnije varijable i opet testirali i usporedili s ovim trecim i dobili da je taj prijasnji s vise varijabli
bio bolji jer smo izgubili informacije micanjem varijabli iz modela

prikaz postotaka za svako

zakljucak
na temelju danih varijabli ne mozemo 100 posto predvidjeti ali je postotak predvidanja X i postotak necega X...
...
colnames(winners) <- colnames(losers) <- c("id", "seed", "entry", "name", "hand", "ht", "ioc", "age", "ace", "df", "svpt", "FirstIn", "FirstWon", "SecondWon", "SvGms", "bpSaved", "bpFaced","rank", "rank_points", "ohb")
...
view(player)
logreg.mdl = glm(won ~ age + ht + seed + hand + ace + df + svpt + FirstIn + FirstWon + SecondWon + SvGms + bpSaved + bpFaced + rank + rank_points + ohb , data = player, family = binomial())
summary(logreg.mdl)
```
```{r analiza modela}
...
```{r}
logreg.reduced = glm(won ~ ht + seed + ace + svpt + FirstWon + SecondWon  + bpSaved + bpFaced , data = player, family = binomial())
summary(logreg.reduced)
```
```{r analiza modela}

yHat <- logreg.reduced$fitted.values > 0.4
tab <- table(player$won, yHat)
...
```{r}
anova(logreg.mdl, logreg.reduced, test = "LRT")
```

```{r}
logreg.reduced2 = glm(won ~ age + ht + seed  + ace + svpt + FirstWon + SecondWon + SvGms + bpSaved + bpFaced + rank + rank_points + ohb , data = player, family = binomial())
summary(logreg.reduced2)
```
```{r analiza modela}

yHat <- logreg.reduced2$fitted.values > 0.4
tab <- table(player$won, yHat)
...
```{r}
anova(logreg.mdl, logreg.reduced2, test = "LRT")
anova(logreg.reduced, logreg.reduced2, test = "LRT")
```
```{r}
logreg.df = glm(won ~ age + ht + seed + hand + ace + svpt + FirstIn + FirstWon + SecondWon + SvGms + bpSaved + bpFaced + rank + rank_points + ohb , data = player, family = binomial())
summary(logreg.df)

anova(logreg.df, logreg.mdl, test="LRT")
```
