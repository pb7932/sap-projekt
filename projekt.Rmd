...
Provjera normalnosti visina top deset najboljih igrača
```{r}
lillie.test(topTen$ht)
```

Provjera noramlnosti visina igrača koji nisu u top deset najboljih
```{r}
lillie.test(notTopTen$ht)
```
...
Provjera normalnosti postotka osvojensoti poena prvim servisom pobjednika
```{r}
lillie.test(p1stWon.w)
```

Provjera normalnosti postotka osvojensoti poena prvim servisom gubitnika
